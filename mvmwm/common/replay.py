...
class Replay:
    def __init__(
        ...
        if self._augment:
            ...
            self._transform = A.ReplayCompose(
                [
        ...
        if load:
            self._complete_eps = load_episodes(load_directory, capacity, load_minlen)
            if len(self._complete_eps) != 0:
                ...
                print("load " + str(len(self._complete_eps)) + " episodes")
            else:
                self._eps_keys = []
        else:
            self._complete_eps, self._eps_keys = {}, []
    ...
    def add_episode(self, episode):
        ...
        filename = save_episode(self._directory, episode, self._save)
        ...
        while True:
            ...
            if self._augment:
                for k, v in chunk.items():
                    if v.dtype == np.uint8:
                        chunk[k] = self._transform(image=v)["image"]
            yield chunk
...
def save_episode(directory, episode, save=True):
    ...
    for filename in filenames:
        try:
            with filename.open("rb") as f:
                episode = np.load(f)
                if len(episode["reward"]) < minlen:
                    print(
                        f"Ignore {str(filename)} due to {str(len(episode['reward']))} is less than {str(minlen)}"
                    )
                    continue
